...
### 2. 使用比较分析
- 从价格来看：
百度：有免费版，个人账号是50万次/累计，企业认证是50万次/每天。付费版是免费调用50万/天，超出后是1.5-2.5每千次。
阿里：每天5万次免费次数，超出后10万次(￥450),50万次 (￥2,000),100万次 (￥3,500)。
腾讯：1～10万次价格为￥50每万次，10～50万次为￥45每万次，50～100为40每万次，100+为35每万次。

百度在价格上较为优惠一段。

### 3. 使用后风险报告
- 手写文字识别中，虽然针对不规则的手写字体进行专项优化，手写中文识别准确率可达90%以上，但是仍然有识别错误的可能，此时会需要用户手动修改识别到的文章错误内容。
- 文章分类中，输出文章的主题一级分类、主题二级分类，如娱乐、社会、音乐、人文、科学、历史、军事、体育、科技、教育等分类结果，若用户的需要更加细致的分类，可能还有所欠缺。例如：用户想要查找情感分类里的亲情。可能无法做到如此细致。
